...
const DocumentCitationViewer = ({ viewer, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col bg-[#060606] text-white">
      <div className="flex items-center justify-between border-b border-white/8 px-6 py-5">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-neutral-500">
            Citation View
          </p>
          <h2 className="mt-2 text-2xl font-semibold">{viewer.documentName}</h2>
        ...
        <button
          type="button"
          onClick={onClose}
          className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-[#181818] px-4 py-2 text-sm font-semibold text-neutral-200 transition hover:border-white/20 hover:bg-[#222222]"
        >
          <X size={16} />
          Exit document view
        </button>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
        <div className="mx-auto max-w-5xl rounded-[28px] border border-white/8 bg-[#111111] p-6 shadow-[0_25px_60px_rgba(0,0,0,0.35)]">
